...
import { REF, SCREENS, VERSIONS, isReady, type ScreenId, type VersionId } from '@/versions/registry'
...
function VersionShell() {
  ...
  else if (screen && !isReady(version.id, screen.id)) body = <Placeholder title={`${version.label} · ${screen.name}`} line={`มี mockup แล้ว (${REF[version.id][screen.id]}) — ทำเป็น prototype ในรอบถัดไป`} />
...
function Screen({ v, s, collapsed }: { v: VersionId; s: ScreenId; collapsed: boolean }) {
  ...
  if (s === 'menu') return v === 'v1' ? <MenuA /> : v === 'v2' ? <MenuB /> : <MenuC />
        ...
        {VERSIONS.map(v => (
          <div key={v.id} className="bg-white border border-ink-200 rounded-[14px] p-4 flex flex-col gap-2.5 shadow-sm">
            ...
            <div className="flex flex-col gap-1 mt-0.5">
              {SCREENS.map(s => isReady(v.id, s.id) ? (
                <button key={s.id} onClick={() => open(v.id, s.id)} className="text-left flex items-center gap-2.5 px-2.5 py-[7px] border border-ink-150 rounded-[9px] bg-white hover:border-ink-400">
                  <i className={`${s.icon} w-4 text-center text-ink-500`} /><span className="flex-1 font-semibold">{s.name}</span><span className="font-display text-[12px] text-ink-400">{REF[v.id][s.id]}</span><i className="fas fa-arrow-right text-[11px] text-ink-400" />
                </button>
              ) : (
                <div key={s.id} className="flex items-center gap-2.5 px-2.5 py-[7px] border border-dashed border-ink-200 rounded-[9px] text-ink-400">
                  <i className={`${s.icon} w-4 text-center`} /><span className="flex-1">{s.name}</span><span className="font-display text-[12px]">{REF[v.id][s.id]}</span><span className="text-[11.5px] font-semibold px-2 py-0.5 rounded-full bg-ink-100 text-ink-500">รอบถัดไป</span>
                </div>
              ))}
            </div>
          </div>
