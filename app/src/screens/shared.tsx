    ...
    <div className="relative">
      <div className={`rounded-[10px] px-3 py-2.5 flex items-center gap-2.5 border ${value ? 'border-warning-500 bg-warning-50' : 'border-ink-150 bg-white'}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className={`text-[12px] flex gap-1.5 items-center ${value ? 'text-warning-700' : 'text-info-700'}`}><i className={`fas ${value ? 'fa-unlink' : 'fa-link'} text-[9px]`} /><span className="truncate">{sub}</span></div>
        </div>
        {value
      ...
      {open && (
        <div className="absolute z-20 left-0 right-0 top-[calc(100%+6px)] bg-white border border-ink-150 rounded-xl shadow-xl p-3 flex flex-col gap-2.5 min-w-[240px]">
          <div className="text-[12px] font-semibold text-ink-600">ตั้งทับเฉพาะ{scope} — เลือก Token หรือสีเอง</div>
  ...
  return (
    <div className="flex-1 grid place-items-center text-center text-ink-500 py-10 px-4 text-[13.5px] leading-relaxed">
      <div><i className="far fa-lightbulb text-xl text-ink-300" /><br /><b className="text-ink-900">{name}</b><br />ยังไม่มีใน mockup — พื้นที่ว่างสำหรับลอง idea<br />
        {onBack && <button onClick={onBack} className="mt-3 h-8 px-3 rounded-lg border border-ink-200 text-ink-700 font-semibold text-[12.5px] hover:bg-ink-50">{backLabel}</button>}</div>
    </div>
  )
}
