...
export function LockPill({ lock, long }: { lock: Lock; long?: boolean }) {
  return <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-[5px] whitespace-nowrap" style={{ background: LOCK_STYLE[lock].bg, color: LOCK_STYLE[lock].fg }}>{lock}{long ? (lock === 'L2' ? ' แก้ค่า' : ' อิสระ') : ''}</span>
}
...
export function Tabs({ id, items, def }: { id: string; items: [string, ReactNode][]; def: string }) {
  ...
  return (
    <div role="tablist" className="flex border-b border-ink-150 px-2 font-medium text-ink-500 text-[14px] flex-none">
      {items.map(([k, label]) => (
...
export function NotInMockup({ what }: { what: string }) {
  return (
    <div className="flex-1 grid place-items-center text-center text-ink-500 p-6 text-[13.5px] leading-relaxed">
...
export function DiffCard({ accept = 'var(--ink-900)', compact = false }: { accept?: string; compact?: boolean }) {
  ...
  return (
    <div className="border border-ink-150 rounded-xl overflow-hidden bg-white">
      <div className="flex items-center gap-2 px-2.5 py-2 border-b border-ink-100 bg-ink-50"><span className="text-[12px] font-bold text-ink-600 tracking-[.04em]">DIFF · 2 การเปลี่ยนแปลง</span><span className="ml-auto text-[12px] text-ink-500">ฉบับร่าง</span></div>
      {AI_PROPOSAL.items.map(it => {
        ...
        return (
          <div key={it.id} className={`p-2.5 flex gap-2 items-center border-b border-ink-100 ${st === 'no' ? 'opacity-45' : ''} ${st === 'ok' ? 'bg-success-50' : ''}`}>
            ...
            <div className="flex-1 min-w-0"><div className={`font-semibold ${st === 'no' ? 'line-through' : ''}`}>{it.title}</div><div className="text-[12px] text-ink-500">{compact ? it.short : it.id === 'hero' ? <><span className="line-through">#dfe7f3</span> → #B07A55 · หัวข้อ "AUTUMN EDIT 2026"</> : it.detail}</div></div>
      ...
      <div className="flex gap-2 p-2.5 bg-ink-50">
        <button onClick={acceptAll} className="flex-1 h-8 rounded-lg text-white font-semibold text-[13px]" style={{ background: accept }}>ยอมรับทั้งหมด</button>
        <button onClick={() => setCompare(compare === 'after' ? 'before' : 'after')} className="h-8 px-3 rounded-lg border border-ink-200 text-[13px] bg-white">{compare === 'after' ? (compact ? 'ก่อน/หลัง' : 'ดูก่อน/หลัง') : 'ดูฉบับร่าง'}</button>
      </div>
    </div>
  )
}
...
export function TierChips({ labels }: { labels: [string, string, string] }) {
  return (
    <div className="flex gap-1.5 mb-2 text-[12px] font-semibold">
