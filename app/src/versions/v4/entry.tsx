...
function Topbar({ children }: { children: React.ReactNode }) {
  return (
    <div className="h-14 bg-white border-b border-ink-150 flex items-center px-6 gap-3 flex-none">
      <span className="font-bold text-title">ปรับแต่งเว็บไซต์</span><span className="text-ink-400">/</span><span className="text-ink-600">เลือกหน้า</span>
    ...
    <div className="grid grid-cols-3 gap-3.5 flex-1 min-h-0">
      {PAGE_GROUPS.map(g => (
        <div key={g.key} className="bg-white border border-ink-150 rounded-[14px] flex flex-col overflow-hidden">
          <div className="py-2.5 px-3.5 border-b border-ink-100 flex items-center gap-2"><i className={`${g.icon} text-ink-500`} /><span className="font-bold">{g.label}</span><span className="font-display text-ink-400">{g.count}</span><span className="ml-auto text-caption text-ink-500">{g.note}</span></div>
        ...
        <div className="rounded-[18px] shadow-md border border-black/5 px-[22px] py-[18px] flex items-center gap-5" style={{ background: '#fff radial-gradient(ellipse 40% 80% at 100% 50%,rgba(230,52,34,.08),transparent 70%)' }}>
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-2.5">
            ...
            <div className="border border-ink-200 rounded-[14px] px-3 py-2.5 flex flex-col gap-2 bg-white">
              <div className="text-body text-ink-400">เช่น "เปลี่ยนแบนเนอร์หน้าแรกเป็นแคมเปญ 11.11 โทนดำ-ทอง"</div>
              <div className="flex items-center gap-2 text-ink-500 text-meta">
                ...
                <span className="ml-auto h-[34px] px-3.5 rounded-[10px] text-white flex items-center gap-2 font-semibold text-body" style={{ background: 'linear-gradient(135deg,var(--red-600),var(--orange-500))' }}>สร้างฉบับร่าง <i className="fas fa-arrow-right text-[11px]" /></span>
              </div>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2.5"><span className="font-bold text-heading">เลือกหน้าที่จะแต่ง</span><div className="h-8 w-[240px] border border-ink-200 rounded-lg bg-white flex items-center px-2.5 gap-2 text-ink-400 ml-1.5 text-meta"><i className="fas fa-search" />ค้นหาหน้า…</div><div className="flex-1" /><span className="text-meta text-ink-500"><span className="text-success-500">●</span> เผยแพร่แล้ว &nbsp; <span className="text-warning-600">●</span> มีฉบับร่าง</span></div>
        <PageGroups withEdit />
      </div>
    </div>
  )
