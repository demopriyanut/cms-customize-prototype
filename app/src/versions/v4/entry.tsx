...
const LockTag = ({ lock }: { lock: PageDoc['lock'] }) => <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-[5px]" style={{ background: LOCK_STYLE[lock].bg, color: LOCK_STYLE[lock].fg }}>{lock}</span>

function Topbar({ children }: { children: React.ReactNode }) {
  return (
    <div className="h-14 bg-white border-b border-ink-150 flex items-center px-6 gap-3 flex-none">
      <span className="font-bold text-[15px]">ปรับแต่งเว็บไซต์</span><span className="text-ink-400">/</span><span className="text-ink-600">เลือกหน้า</span>
    ...
    <div className="grid grid-cols-3 gap-3.5 flex-1 min-h-0">
      {PAGE_GROUPS.map(g => (
        <div key={g.key} className="bg-white border border-ink-150 rounded-[14px] flex flex-col overflow-hidden">
          <div className="py-2.5 px-3.5 border-b border-ink-100 flex items-center gap-2"><i className={`${g.icon} text-ink-500`} /><span className="font-bold">{g.label}</span><span className="font-display text-ink-400">{g.count}</span><span className="ml-auto text-[11px] text-ink-500">{g.note}</span></div>
          <div className="p-1.5 flex flex-col gap-[3px] overflow-auto">
            {pages.filter(p => p.group === g.key).map(p => (
              <button key={p.id} onClick={() => open(p)} className={`w-full text-left flex items-center gap-2.5 px-2 py-[7px] rounded-[10px] ${p.lock === 'L3' ? 'opacity-50' : 'hover:bg-ink-50'}`}>
                ...
                <span className="flex-1 min-w-0"><span className="block font-semibold text-[12.5px] truncate">{p.name}</span><span className="block text-[11px] text-ink-500">{p.path} · {p.when}</span></span>
                {p.lock === 'L3' && <span className="text-[10px] text-ink-500">อ่านอย่างเดียว</span>}
                <LockTag lock={p.lock} />
                <span className="w-2 h-2 rounded-full flex-none" style={{ background: dot(p.status) }} title={p.status} />
                {withEdit && <span className="text-[11px] font-semibold border border-ink-200 rounded-md px-2 py-[3px]">แก้ไข</span>}
              </button>
            ))}
          </div>
        </div>
...
export function EntryV4(_: { collapsed?: boolean }) {
  return (
    <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-cream text-[13px]">
      <Topbar>
        <span className="flex items-center gap-1.5 text-[12px] text-ink-500"><span className="w-2 h-2 rounded-full bg-warning-500" />2 หน้ามีฉบับร่างรอเผยแพร่</span>
        <ViewSite />
        <NewPageBtn className="h-9 rounded-lg bg-ink-900 text-white flex items-center px-3 gap-2 font-semibold" />
      </Topbar>
      <div className="flex-1 overflow-auto px-6 pt-[22px] pb-24 flex flex-col gap-[18px]">
        <div className="rounded-[18px] shadow-md border border-black/5 px-[22px] py-[18px] flex items-center gap-5" style={{ background: '#fff radial-gradient(ellipse 40% 80% at 100% 50%,rgba(230,52,34,.08),transparent 70%)' }}>
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-2.5">
            <div><div className="font-bold text-lg">สวัสดีครับ admin — จะให้ช่วยแต่งอะไรดี?</div><div className="text-[12px] text-ink-500">พิมพ์สั่ง หรือวางรูปเว็บที่ชอบให้ผมล้อโครงตาม · ทุกอย่างลงฉบับร่างก่อน คุณกดเผยแพร่เอง</div></div>
            <div className="border border-ink-200 rounded-[14px] px-3 py-2.5 flex flex-col gap-2 bg-white">
              <div className="text-[13.5px] text-ink-400">เช่น "เปลี่ยนแบนเนอร์หน้าแรกเป็นแคมเปญ 11.11 โทนดำ-ทอง"</div>
              <div className="flex items-center gap-2 text-ink-500 text-[12px]">
                ...
                <span className="ml-auto h-[34px] px-3.5 rounded-[10px] text-white flex items-center gap-2 font-semibold text-[12.5px]" style={{ background: 'linear-gradient(135deg,var(--red-600),var(--orange-500))' }}>สร้างฉบับร่าง <i className="fas fa-arrow-right text-[11px]" /></span>
              </div>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2.5"><span className="font-bold text-sm">เลือกหน้าที่จะแต่ง</span><div className="h-8 w-[240px] border border-ink-200 rounded-lg bg-white flex items-center px-2.5 gap-2 text-ink-400 ml-1.5 text-[12px]"><i className="fas fa-search" />ค้นหาหน้า…</div><div className="flex-1" /><span className="text-[12px] text-ink-500"><span className="text-success-500">●</span> เผยแพร่แล้ว &nbsp; <span className="text-warning-600">●</span> มีฉบับร่าง</span></div>
        <PageGroups withEdit />
      </div>
    </div>
  )
