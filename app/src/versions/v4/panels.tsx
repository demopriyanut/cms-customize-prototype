/* V4 — cloned from components/editor/panels.tsx on 2026-09-24 so V4 owns it (type standard: text-display/title/heading/body/meta/caption). Only V4 uses this file. */
...
import { useDrag } from '@/components/editor/drag'
...
export function LayerList({ site, page, variant }: { site: SiteDoc; page: PageDoc; variant: 'card' | 'compact' }) {
  ...
  const row = (s: Section, zone: string, index: number) => {
    ...
    if (variant === 'compact') return (
      <div key={s.id} {...common} onClick={() => select(s.id)} role="button" tabIndex={0} aria-pressed={sel} onKeyDown={e => e.key === 'Enter' && select(s.id)}
        ...
        <span className="text-caption font-bold px-[5px] py-px rounded whitespace-nowrap" style={{ background: rs.bg, color: rs.fg }}>{locked ? (s.role === 'global' ? 'ทุกหน้า' : 'ระบบ') : s.origin ? 'AI' : page.lock}</span>
      </div>
    )
    return (
      <div key={s.id} {...common} onClick={() => select(s.id)} role="button" tabIndex={0} aria-pressed={sel} onKeyDown={e => e.key === 'Enter' && select(s.id)}
  ...
  for (const z of zones) {
    ...
    if (page.lock !== 'L0') rows.push(<div key={'zh-' + z.id} data-zone-label={z.label} className="text-caption font-semibold tracking-[.04em] px-1 pt-1.5" style={{ color: z.insert ? ROLE_STYLE.slot.fg : 'var(--ink-500)' }}>{z.insert ? <i className="fas fa-plus-square mr-1" /> : <i className="fas fa-lock mr-1" />}{z.label}{z.closedNote ? ` · ${z.closedNote}` : ''}</div>)
    if (inZone.length === 0) {
      rows.push(z.insert
    ...
    <div ref={ref} className={`flex flex-col ${variant === 'card' ? 'gap-2' : 'gap-1'}`}>
      ...
      {drag.state && (
        <>
          ...
          <div className="fixed z-[90] pointer-events-none text-body max-w-[300px]" style={{ left: drag.state.x + 14, top: drag.state.y + 10 }}>
...
export function LibraryPanel({ page, className = '' }: { page: PageDoc; className?: string }) {
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-xl border border-black/5 p-4 w-[312px] flex flex-col gap-3 z-30 ${className}`} role="dialog" aria-label="คลัง">
      <div className="flex items-center justify-between"><b className="text-heading">คลัง</b><button onClick={() => setLibrary(false)} aria-label="ปิดคลัง" className="w-7 h-7 rounded-lg hover:bg-ink-100 text-ink-500"><i className="fas fa-times" /></button></div>
      {!canInsert ? (
        <div className="text-body text-ink-600 leading-relaxed bg-warning-100/60 rounded-lg p-2.5"><i className="fas fa-lock mr-1" />หน้านี้ระดับ <b>{page.lock}</b> — {LOCK_TEXT[page.lock].rule}</div>
      ) : <>
        <div className="text-meta text-ink-500">เลือกก่อน แล้วคลิกช่อง “วางที่นี่” บนหน้าเว็บ</div>
...
export function PlacingBar({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={`bg-info-700 text-white rounded-full pl-3 pr-1.5 py-1.5 flex items-center gap-2 text-body shadow-lg z-40 ${className}`} role="status">
...
export function PageRuleBanner({ page, className = '', style }: { page: PageDoc; className?: string; style?: React.CSSProperties }) {
  ...
  return (
    <div className={`rounded-xl px-4 py-3 text-body leading-relaxed flex items-start gap-2.5 border ${className}`} style={{ background: LOCK_STYLE[page.lock].bg, color: LOCK_STYLE[page.lock].fg, borderColor: 'rgba(0,0,0,.06)', ...style }}>
      ...
      {s.role === 'free' && <>
        <Group title="ระยะห่าง">
          <div className="flex gap-1 bg-ink-100 rounded-lg p-[3px]">{(['S', 'M', 'L'] as const).map(k => {
            ...
            return <button key={k} disabled={readOnly} onClick={() => st().setStyle(s.id, { spacing: k }, `ระยะห่าง ${k} · ${s.name}`)} className={`flex-1 h-7 rounded-md text-body ${on ? 'bg-white shadow-xs font-semibold' : 'text-ink-500'}`}>{k === 'S' ? 'แคบ' : k === 'M' ? 'ปกติ' : 'กว้าง'}</button>
          })}</div>
        </Group>
...
function Head({ s }: { s: Section }) {
  ...
  return <div className="flex items-center gap-2"><span className="w-8 h-8 rounded-lg grid place-items-center" style={{ background: rs.bg, color: rs.fg }}><i className={SECTION_ICON[s.type]} /></span><div className="flex-1 min-w-0"><div className="font-bold text-title truncate">{s.name}</div><div className="text-meta" style={{ color: rs.fg }}>{rs.label}</div></div></div>
}
function Group({ title, children }: { title: string; children: React.ReactNode }) {
  return <div className="flex flex-col gap-2"><div className="text-meta font-semibold text-ink-600 tracking-[.02em]">{title}</div>{children}</div>
}
