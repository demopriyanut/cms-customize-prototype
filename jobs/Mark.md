...
| `app/src/index.css` (รอบ sidebar V4) | `.nav-scroll` แถบเลื่อนบางใน sidebar (ใช้แค่ V4) |

## บันทึก (ใหม่อยู่บน)

| วันที่ | ทำอะไร | commit | ค้าง / ส่งต่อ |
|---|---|---|---|
| 2026-09-24 | V4 sidebar แก้ 5 ข้อ: ไอคอนเส้นแบบ Lucide ชุดเดียว · หัวกลุ่มภาษาเดียว · เมนูที่เปิดอยู่ = พื้นแดง CI ตัวขาวหนา (ไม่มีเส้นหน้า) · hover ต่างจากที่เลือก · เมนูที่ยังไม่มีหน้าจอ = ปิดใช้งาน + บอกเหตุผล · รายการเยื้องจากหัวกลุ่ม · ป้าย "เลื่อนดูเมนูเพิ่ม" · V4 ไม่มีปุ่มดำ: สร้างหน้าใหม่ = แดง CI · ปุ่มรองขาวขอบเทา · ที่เลือก/กดค้าง = ส้ม · ปุ่มผู้ช่วย = ไล่สี | (commit นี้) | review ทั้งชุดส่งผู้ใช้แล้ว (ปุ่มจัดการ/ปรับแต่งจม · สถานะเลือก 4 แบบ · ปุ่มรองไม่เหมือนกัน · ป้ายแผงขวา 3 ระบบ) รอตัดสิน · Page Layouts: ลองรายการเดียวแล้วเอากลับ รอข้อเสนอ flow ผู้ช่วย |
